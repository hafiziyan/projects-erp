...
const AppSidebar: React.FC = () => {
  ...
  const [merchantName, setMerchantName] = useState<string>("ERP System");

  useEffect(() => {
    const active = getActiveMerchant();
    setUserRole(active?.role || null);
    setMerchantName(active?.merchantName || "ERP System");
  }, []);
          ...
          {(isExpanded || isHovered || isMobileOpen) ? (
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                <h1 className="text-lg font-black text-gray-900 dark:text-white group-hover:text-brand-600 dark:group-hover:text-brand-400 transition-colors">
                  {merchantName}
                </h1>
                <p className="text-[10px] font-semibold uppercase tracking-wider text-gray-400">ERP System</p>
              </div>
            </div>
          ) : (
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-brand-500 to-brand-700 shadow-lg shadow-brand-500/30">
