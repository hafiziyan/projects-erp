...
const createPurchaseSchema = zod_1.z.object({
    ...
    items: zod_1.z
        ...
        newSalePrice: zod_1.z.union([zod_1.z.string(), zod_1.z.number()]).optional(), // Field baru
    }))
...
function getMerchantIdFromHeader(req) {
    ...
    if (!merchantIdValue || !/^\d+$/.test(merchantIdValue))
        return null;
...
async function createPurchase(req, res) {
    try {
        ...
        if (normalizedItems.some((item) => item.productId === null ||
            item.quantity === null ||
            item.cost === null ||
            item.quantity <= 0 ||
            item.cost < 0)) {
        ...
        const invoiceNumber = parsed.data.invoiceNumber && parsed.data.invoiceNumber.trim() !== ''
        ...
        const purchaseDetail = await prisma_1.prisma.purchase.findUnique({
            where: { id: result.id },
            include: {
                user: true,
