...
async function getDashboardSummary(req, res) {
    try {
        ...
        const [totalProducts, totalCategories, totalUsers, totalStocks, lowStockProducts, todaySalesAgg, monthSalesAgg, monthPurchaseAgg, recentSales, lowStockList,] = await Promise.all([
            ...
            prisma_1.prisma.merchantUser.count({
                where: {
                    merchantId,
                    status: 'active',
                },
            }),
            prisma_1.prisma.stock.aggregate({
                where: {
                    merchantId,
                },
                _sum: {
        ...
        const lowStockCount = lowStockProducts.filter((item) => item.actualQuantity <= item.product.reorderPoint).length;
